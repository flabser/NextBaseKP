...
class QuerySave extends _FormQuerySave {
	...
	public void doQuerySave(_Session session, _Document doc, _WebFormData webFormData, String lang) {
		...
		if (blocksCollection.getStatus() == _CoordStatusType.DRAFT){
			...
			if (doc.getViewNumber()== -1 && blocksCollection.getStatus() != _CoordStatusType.DRAFT){
                ...
                int num = db.getRegNumber("orderprj_" + webFormData.getValueSilently("ordtype"))
            ...
            vt = 'Проект приказа № ' + doc.getValueString("vn") + ' ' + _Helper.getDateAsStringShort(dDate) + ' ' + author.getShortName() + ' ' + doc.getValueString('briefcontent')
		}
	...
	def validateprj(_WebFormData webFormData){
		...
		if (webFormData.getValueSilently("ordtype") == "" || !webFormData.containsField("ordtype")){
			localizedMsgBox("Поле \"Тип приказа\" не выбрано.")
			return false
		}
